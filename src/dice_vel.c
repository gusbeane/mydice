...
// Cartesian grid. The method was adapted from the discussion found in Hockney
// and Eastwood, "Computer Simulation Using Particles," 1981.
int set_turbulent_grid_stream(stream *st, int component) {
	unsigned long int ii, start, end, l;
	// Loop variables
	int i, j, k;
	// Nodes coordinates
	...
	// Allocate grid storage variables
	if (!(kernel = calloc(pow(st->ngrid_turb_padded,3),sizeof(fftw_complex)))) {
		fprintf(stderr,"Unable to allocate space for kernel's function.\n");
		return -1;
	}
	if (!(turbulence = calloc(pow(st->ngrid_turb_padded,3),sizeof(fftw_complex)))) {
		fprintf(stderr,"Unable to allocate space for turbulence buffer.\n");
		return -1;
	}
	
	if (!(kernel_grid=calloc(st->ngrid_turb_padded,sizeof(double *)))) {
		fprintf(stderr,"Unable to create kernel's function x axis.\n");
		return -1;
	}
	// kernel grid allocation
	...
	for (i = 0; i < st->ngrid_turb_padded; ++i) {
	        // y-axis
	        if (!(kernel_grid[i] = calloc(st->ngrid_turb_padded,sizeof(double *)))) {
			fprintf(stderr,"Unable to create kernel's function y axis.\n");
           		return -1;
        	}
	        // z-axis
        	for (j = 0; j < st->ngrid_turb_padded; ++j) {
			if (!(kernel_grid[i][j] = calloc(st->ngrid_turb_padded,sizeof(double)))) {
				fprintf(stderr,"Unable to create kernel's function z axis.\n");
				return -1;
			}
		}
	...
	// Define which particles we should use in the turbulence computation
	// We use all the particles
	start 	= st->comp_start_part[component];
	end 	= st->comp_start_part[component]+st->comp_npart[component];

	// Allocate the fftw complex output value and the fftw dft plan.
	fft_turbulence	= fftw_plan_dft_3d(st->ngrid_turb_padded,st->ngrid_turb_padded,st->ngrid_turb_padded,turbulence,turbulence,FFTW_FORWARD,FFTW_ESTIMATE);
	fft_kernel		= fftw_plan_dft_3d(st->ngrid_turb_padded,st->ngrid_turb_padded,st->ngrid_turb_padded,kernel,kernel,FFTW_FORWARD,FFTW_ESTIMATE);
	
	// Normalization constant
	// See FFTW reference guide for more details
	n = (int)pow(st->ngrid_turb_padded,3.0);
	
	// Check for bad grids
	if (st->ngrid_turb_padded <= 0) {
		fprintf(stderr,"\t\tGrid dimensions must be greater than zero! (ngrid=%d)\n",st->ngrid_turb_padded);
		return -1;
	}
	...
	space_x = st->space_turb[0];
	space_y = st->space_turb[1];
	space_z = st->space_turb[2];
	// Print the turbulence in the xy-plane for z = 0 if the option is set.
	//if (verbose) printf("/////\t\t-Grid cell spacings [kpc]: dx = %.3f dy = %.3f dz = %.3f\n",space_x,space_y,space_z);
	fflush(stdout);
	...
	for (i = 0; i < st->ngrid_turb_padded; ++i) {
		for (j = 0; j < st->ngrid_turb_padded; ++j) {
			for (k = 0; k < st->ngrid_turb_padded; ++k) {
				st->turbulence[i][j][k] = gsl_ran_gaussian(r[0],1.0);
			}
		}
	}
	...
	// discussion. The octants start in the lower left at (p,q) = (0,0) and progress
	// counter clockwise.
	for (i = 0; i < st->ngrid_turb_padded/2; ++i) {
	    for (j = 0; j < st->ngrid_turb_padded/2; ++j) {
			...
			for (k = 0; k < st->ngrid_turb_padded/2; ++k) {
		        ...
		        // Octant 2
                kernel_grid[st->ngrid_turb_padded-1-i][j][k] = kernel_grid[i][j][k];
                // Octant 3
                kernel_grid[st->ngrid_turb_padded-1-i][st->ngrid_turb_padded-1-j][k] = kernel_grid[i][j][k];
                // Octant 4
                kernel_grid[i][st->ngrid_turb_padded-1-j][k] = kernel_grid[i][j][k];
                // Octant 5
                kernel_grid[i][j][st->ngrid_turb_padded-1-k] = kernel_grid[i][j][k];
                // Octant 6
                kernel_grid[st->ngrid_turb_padded-1-i][j][st->ngrid_turb_padded-1-k] = kernel_grid[i][j][k];
                // Octant 7
                kernel_grid[st->ngrid_turb_padded-1-i][st->ngrid_turb_padded-1-j][st->ngrid_turb_padded-1-k] = kernel_grid[i][j][k];
                // Octant 8
                kernel_grid[i][st->ngrid_turb_padded-1-j][st->ngrid_turb_padded-1-k] = kernel_grid[i][j][k];
			}
		}
	}
	...
	for (i = 0; i < st->ngrid_turb_padded; ++i) {
	    for (j = 0; j < st->ngrid_turb_padded; ++j) {
			for (k = 0; k < st->ngrid_turb_padded; ++k) {
		       	kernel[l] = kernel_grid[i][j][k];
		       	turbulence[l] = st->turbulence[i][j][k];
				l++;
			}
		}
	}
	...
	fftw_destroy_plan(fft_kernel);
	// Allocating memory for the inverse fourier computation
	fftinv_turbulence = fftw_plan_dft_3d(st->ngrid_turb_padded,st->ngrid_turb_padded,st->ngrid_turb_padded,turbulence,turbulence,FFTW_BACKWARD,FFTW_ESTIMATE);
	// Multiply the density by kernel's function to find the k-space turbulence and
	// invert for the real potenital. Second, normalize the system and, finally,
	// put the turbulence information into the grid.
	for (i = 0; i < n; ++i) {
	...
	for (i = 0; i < st->ngrid_turb_padded; ++i) {
	        for (j = 0; j < st->ngrid_turb_padded; ++j) {
				for (k = 0; k < st->ngrid_turb_padded; ++k) {
					// Fix the grid info
					st->turbulence[i][j][k] = turbulence[l]/stddev*st->comp_turb_sigma[component];
					l++;
				}
		}
	}
...
double galaxy_turbulence_func(galaxy *gal, double x, double y, double z) {
	...
	double a, dx, dy, dz, tx, ty, tz, turbulence;
...
// If the point lies off of the particle mesh, it approximates the potential
// as a function of 1/r.
double stream_turbulence_func(stream *st, double x, double y, double z) {
	...
	// Scale the coordinates
	xtemp = x/st->space_turb[0] + ((double)(st->ngrid_turb_padded/2)-0.5-0.5);
	ytemp = y/st->space_turb[1] + ((double)(st->ngrid_turb_padded/2)-0.5-0.5);
	ztemp = z/st->space_turb[2] + ((double)(st->ngrid_turb_padded/2)-0.5-0.5);
	offset = 0;
	// Determine the parent node.
	node_x = floor(xtemp);
	node_y = floor(ytemp);
	...
	if (xtemp == (double) node_y && ytemp == (double) node_y && ztemp == (double) node_z) {
		// If (x,y,z) is a grid point, return its potential.
		turbulence = st->turbulence[node_x][node_y][node_z];
	} else {
		// If (x,y,z) is not a grid point, use the CIC
		// interpolation function to calculate the potential.
		turb1 = st->turbulence[node_x][node_y][node_z];
		turb2 = st->turbulence[node_x+1][node_y][node_z];
		turb3 = st->turbulence[node_x][node_y+1][node_z];
		turb4 = st->turbulence[node_x][node_y][node_z+1];
		turb5 = st->turbulence[node_x][node_y+1][node_z+1];
		turb6 = st->turbulence[node_x+1][node_y+1][node_z];
		turb7 = st->turbulence[node_x+1][node_y][node_z+1];
		turb8 = st->turbulence[node_x+1][node_y+1][node_z+1];
		// CIC fractions
		dx = 1.0 - (xtemp - (double) node_x);
		dy = 1.0 - (ytemp - (double) node_y);
		dz = 1.0 - (ztemp - (double) node_z);
